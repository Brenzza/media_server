...
read -sp "Enter Filebrowser ADMIN_PASSWORD: " filebrowser_admin_password
echo ""
...
EOL
    ...
    if [[ "$service_name" == "filebrowser" ]]; then
        sed -i "s|FILEBROWSER_ADMIN_PASSWORD=.*|FILEBROWSER_ADMIN_PASSWORD=$filebrowser_admin_password|" "$compose_file"
    fi
